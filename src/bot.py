...
def _format_info(info: CourseInfo) -> str:
    kind_map = {
        "course": "Course",
    ...
    parts: list[str] = ["<b>Coursera summary</b>"]

    if info.title:
        parts.append(f"<b>Title:</b> {_escape_html(info.title)}")

    parts.append(f"<b>Type:</b> {_escape_html(kind_map.get(info.kind, info.kind))}")
    ...
    if info.total_hours is not None:
        ...
        parts.append(f"<b>Workload hint:</b> {_escape_html(info.workload_hint)}")

    if info.course_count is not None:
        parts.append(f"<b>Courses in series:</b> {info.course_count}")

    parts.append(f"<b>Link:</b> {_escape_html(info.url)}")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await _safe_reply(update, START_TEXT, parse_mode=ParseMode.MARKDOWN)

async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await _safe_reply(update, HELP_TEXT, parse_mode=ParseMode.MARKDOWN)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not canon_urls:
        await _safe_reply(update, NO_LINK_TEXT)
        return
    ...
    for msg in results:
        await _safe_reply(update, msg, parse_mode=ParseMode.HTML)

async def on_error(update: object, context: ContextTypes.DEFAULT_TYPE) -> None:
    err = context.error
    if isinstance(err, Conflict):
        logger.error("Telegram Conflict (two instances). Use webhook mode on Render to avoid this.")
        return
    logger.exception("Unhandled error: %s", err)
    if isinstance(update, Update) and update.message:
        await _safe_reply(update, "❌ Unexpected error. Please try again.")
...
def main() -> None:
    ...
    # Polling fallback. Keep Render Web Service alive by holding a port.
